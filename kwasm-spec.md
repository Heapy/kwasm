...
`[TCK-6]` Differential execution: modules generated by `wasm-smith` (<https://github.com/bytecodealliance/wasm-tools/tree/main/crates/wasm-smith>, CLI `wasm-tools smith`) run against the reference interpreter and/or wasmtime, and additionally against the wasm3 interpreter (<https://github.com/wasm3/wasm3>) built from source under two pinned refs (stable tag + a frozen `main` commit) for triangulation; any divergence in result/trap kind is a bug. wasm3 abstains from f32/f64 invocations because its CLI prints floats via lossy `%.7g`/`%.15g` formatting, so abstained engines never cause a divergence on their own. Nightly, with corpus minimization.
